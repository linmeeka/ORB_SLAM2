...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            // 处理关键帧，计算关键帧特征点的BoW映射，将关键帧插入地图
            // 更新上一帧的MP， KF间的连接关系，插入关键帧
            ProcessNewKeyFrame();
    ...
    for(size_t i=0; i<vpMapPointMatches.size(); i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                ...
                // 如果这个MP不在currentKF里，这时只是KF能观测到MP
