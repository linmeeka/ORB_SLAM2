    ...
    else// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                // 运动模型是空的或刚完成重定位
                // mCurrentFrame.mnId<mnLastRelocFrameId+2这个判断不应该有， 表示刚重定位成功
                // 应该只要mVelocity不为空，就优先选择TrackWithMotionModel
                // mnLastRelocFrameId上一次重定位的那一帧
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2) // 1.跟踪参考帧
                {
                    // 将上一帧的位姿作为当前帧的初始位姿
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    bOKReloc = Relocalization();// 若定位成功，优先选择定位的RT
        ...
        // 局部地图跟踪
        // 步骤2.2：在帧间匹配得到初始的姿态后，现在对local map进行跟踪得到更多的匹配（产生新的地图点， 若不，则随着跟踪进行，地图点会越来越少），并优化当前位姿
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    int nmatches = matcher. (mpReferenceKF,mCurrentFrame,vpMapPointMatches);
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        // 如果这个关键点没有生成MP或者没有被观测到，则为它生成一个MP
        MapPoint* pMP = mLastFrame.mvpMapPoints[i];
        if(!pMP)
            bCreateNew = true;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                if(bCreateNew)
                {
                    ...
                    pNewMP->AddObservation(pKF,i); //关键帧和特征点的双向观测关系
                    pKF->AddMapPoint(pNewMP,i); // 
                    pNewMP->ComputeDistinctiveDescriptors(); // MP的描述子，选择观测到它的关键帧的描述子的中值，距离其他关键帧的描述子最短的
                    pNewMP->UpdateNormalAndDepth(); // 主要观测方向
                    mpMap->AddMapPoint(pNewMP); // MP加入地图数据库
     ...
     // 从数KF据库里提取这个KF的引用，放进当前的localMap
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    // 步骤1：遍历当前帧的MapPoints，记录所有能观测到当前帧MapPoints的关键帧，能观测到当前帧MP的KF，以及它们观测到MP的个数
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            if(!pMP->isBad())
            {
                ...
                // 观测到该MapPoint的所有KF和该MapPoint在KF中的索引
...
bool Tracking::Relocalization()
{
    ...
    // If enough matches are found we setup a PnP solver
    // 每一个候选关键帧有对应的：PNPsolver，装MP的vector，是否丢弃的flag
